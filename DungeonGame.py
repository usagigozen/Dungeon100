...
class DungeonGame:
    ...
    def update(self):
        ...
        if self.scene == SNO_LEVEL_UP:
            ...
            """成長選択モードの処理"""
            if pyxel.btnp(pyxel.KEY_A):  # 攻撃力アップ
                self.handle_level_up("ATK")
                self.scene = SNO_SEARCH
            elif pyxel.btnp(pyxel.KEY_D):  # 防御力アップ
                self.handle_level_up("DEF")
                self.scene = SNO_SEARCH
            elif pyxel.btnp(pyxel.KEY_H):  # HP回復
    ...
    def draw(self):
        ...
        # Aボタン（青丸）
        pyxel.circ(pyxel.width - 45, pyxel.height - 20, 6, pyxel.COLOR_LIGHT_BLUE)
        pyxel.text(pyxel.width - 48, pyxel.height - 22, "A", pyxel.COLOR_WHITE)

        # Dボタン（赤丸）
        pyxel.circ(pyxel.width - 30, pyxel.height - 20, 6, pyxel.COLOR_RED)
        pyxel.text(pyxel.width - 33, pyxel.height - 22, "D", pyxel.COLOR_WHITE)

        # Hボタン（赤丸）
        pyxel.circ(pyxel.width - 15, pyxel.height - 20, 6, pyxel.COLOR_RED)
        pyxel.text(pyxel.width - 18, pyxel.height - 22, "H", pyxel.COLOR_WHITE)
